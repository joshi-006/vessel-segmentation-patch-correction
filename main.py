...
def main():
    ...
    models = load_models(MODEL_DIR, device, midl_repo_path=MIDL_REPO_PATH)
    ...
    plot_adaptive_efficiency(adaptive_df, details_df, DELTA_THRESHOLD, OUTPUT_DIR)
    ...
    print_hard_region_analysis(dice_before_list, dice_after_list)

    # ── Evaluation beyond mean Dice (worst-image / bottom-20% / high-MI) ──────
    mean_mi_per_img = np.array(
        [np.clip(mc_cache[n]["mi"], 0, 1).mean() for n in test_images]
    )
    before_np, delta_np = print_evaluation_beyond_mean_dice(
        dice_before_list, dice_after_list, mean_mi_per_img
    )
    ...
    for img_name in test_images:
        ...
        patch_dice_before.append(compute_metrics(before_mask[region_mask], gt[region_mask])["dice"])
        patch_dice_after.append(compute_metrics(after_mask[region_mask], gt[region_mask])["dice"])
    ...
    plot_region_improvement_scatter(before_np, delta_np, mean_mi_per_img, OUTPUT_DIR)

    print("\nKey narrative:")
    print("  Global Dice changes are modest because only ~5-10% of pixels are corrected.")
    print("  Patch-level Dice (metric 4) and hard-region Dice directly")
    ...
    plot_error_maps(test_images, cached_preds, cached_gts, corrected_masks,
                     BEST_THRESH, OUTPUT_DIR, n_show=n_show)
